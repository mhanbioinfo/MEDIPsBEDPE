...
## edited by: Ming Han
## allows for leanBEDPE input in addition to BAM input
            ...
            dataset = NULL, uniq = 1e-3, ROI = NULL, isSecondaryAlignment = FALSE, simpleCigar=TRUE)
  {
    ...
    if (bam) {
      ...
      if (bamindex & (!is.null(chr.select) | !is.null(ROI))) {
        ...
        scanParam = ScanBamParam(flag = scanFlag, simpleCigar= simpleCigar,
                                 what = c("rname", "pos", "strand", "qwidth", "isize", "mpos"), which = sel)
      }
      else {
        cat("Reading bam alignment", fileName, "\n")
        scanParam = ScanBamParam(flag = scanFlag, simpleCigar = simpleCigar,
                                 what = c("rname", "pos", "strand", "qwidth", "isize", "mpos"))
      }
      ...
      regions = do.call(rbind, lapply(regions, as.data.frame,
                                      stringsAsFactors = F))
      regions
      write.table(regions, paste0(data.dir, "CMP-01-02-cfDNA-03.regions.tsv"), sep='\t', quote=F, row.names = F, col.names = F)

    } else if (bedpe) {
      cat("Reading bedpe file: ", fileName, "\n")
      regions =
        read.table(file = paste(path, fileName, sep = "/"),
                   header = F, sep = "\t")
